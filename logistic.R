...
log2 = glm(test~pregnant+glucose+bmi+diabetes,data = clean.pima,family = binomial)
summary(log2)
anova(log1,log2, test = "Chisq")

plot(predict(logm,dat),residuals(logm),xlab = 'predicted values',ylab = 'residuals', main = 'Residual Plot')
# Since the outcome can only be 0 or 1, the deviances cannot be used as an indication of fit
